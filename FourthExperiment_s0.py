...
FracMFG = MFGSolver1D(mesh,kappa,0,nu)
FracMFG2 = MFGSolver1D(mesh,0,kappa,nu)

n = 8
s =  np.exp(-np.linspace(2, 20, n))
print(s)
...
plt.loglog(s,err_u_max, label = r'$\lVert u^s_k - u_k \rVert_{L^\infty}$', marker = 'o')
plt.loglog(s,err_m_max, label = r'$\lVert m^s_k - m_k \rVert_{L^\infty}$', marker = 'o')
plt.loglog(s, ((s)/(s[0]))*0.5*(err_u_max[0]+err_m_max[0]),'--', label = r'Linear line: $s$')
plt.gca().invert_xaxis()  # Optional: shows values approaching 1 from the left
plt.xlabel(r'$s$', fontsize = labelsize)
plt.ylabel('Error',fontsize = labelsize)
plt.title(r'Continuity of solutions $(u_k^s,m_k^s)$ as $s\to 0$',fontsize = labelsize)
#plt.grid(True)
plt.legend(fontsize = labelsize-2)
plt.show()

plt.figure(figsize = (5,5))
plt.loglog(s,err_u_L2, label = r'$\lVert u^s_k - u_k \rVert_{L^2}$', marker = 'o')
plt.loglog(s,err_u_H1, label = r'$\lVert u^s_k - u_k \rVert_{H^1}$', marker = 'o')
plt.loglog(s,err_m_L2, label = r'$\lVert m^s_k - m_k \rVert_{L^2}$', marker = 'o')
plt.loglog(s,err_m_H1, label = r'$\lVert m^s_k - m_k \rVert_{H^1}$', marker = 'o')
plt.loglog(s, ((s)/(s[0]))*0.25*(err_u_L2[0]+err_u_H1[0]+err_m_L2[0]+err_m_H1[0]),'--', label = r'Linear line: $s$')
plt.gca().invert_xaxis()  # Optional: shows values approaching 1 from the left
plt.xlabel(r'$s$', fontsize = labelsize)
plt.ylabel('Error', fontsize = labelsize)
plt.title(r'Continuity of solutions $(u_k^s,m_k^s)$ as $s\to 0$', fontsize = labelsize)
